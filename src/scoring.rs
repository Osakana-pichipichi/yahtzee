...
pub fn scoring(b: &Boxes, dice: &[u32]) -> u32 {
	if dice.len() != Hand::DICE_NUM {
		panic!("A hand needs {} dice, but your hand has only {} dice.",
			   Hand::DICE_NUM, dice.len());
	}
	match *b {
...
fn aces(dice: &[u32]) -> u32 {
	return upper_section_scoring(dice, 1);
}

fn twos(dice: &[u32]) -> u32 {
	return upper_section_scoring(dice, 2);
}

fn threes(dice: &[u32]) -> u32 {
	return upper_section_scoring(dice, 3);
}

fn fours(dice: &[u32]) -> u32 {
	return upper_section_scoring(dice, 4);
}

fn fives(dice: &[u32]) -> u32 {
	return upper_section_scoring(dice, 5);
}

fn sixes(dice: &[u32]) -> u32 {
	return upper_section_scoring(dice, 6);
}

fn n_of_a_kind(dice: &[u32], n: u32) -> u32 {
	let n: usize = n as usize;
	for i in 0..=(Hand::DICE_NUM - n) {
		let target: u32 = dice[i];
		if dice.iter().filter(|&d| *d == target).count() >= n {
...
fn three_of_a_kind(dice: &[u32]) -> u32 {
	return n_of_a_kind(dice, 3);
}

fn four_of_a_kind(dice: &[u32]) -> u32 {
	return n_of_a_kind(dice, 4);
}
...
fn small_straight(dice: &[u32]) -> u32 {
	...
	for i in 0..=(sorted_dice.len() - 4) {
		let init: u32 = sorted_dice[i];
		if sorted_dice[i..(i + 4)] == array![i => init + (i as u32); 4] {
...
fn upper_section_scoring(dice: &[u32], spots: u32) -> u32 {
	return (dice.iter().filter(|&d| *d == spots).count() as u32) * spots;
}
